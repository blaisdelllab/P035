...
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 30 09:55:11 2025

@author: kayleyozimac

#This is the code for Kayley Ozimac's mini project (P035f) for her 'FOAM' project

This is a essentially a replication of P035c, however the comparison stimuli 
are now familiarized as a start key to a relational discrimination task in either 
the left or right comparison lcoations instead of in the center of the screen
as before.
...
class ExperimenterControlPanel(object):
    ...
    def __init__(self):
        ...
        self.control_window.title("P035f Control Panel")
        ...
        # Exp phases
        self.experimental_phase_titles = ["Phase 1 (Familiarization & choice task)", 
...
class MainScreen(object):
    ...
    def __init__(self, subject_ID, record_data, data_folder_directory,
        ...
        self.root = Toplevel() 
        self.root.title(f"P035f {self.exp_phase_name}: ") # this is the title of the window
        ...
        header_list = [
            ...
            "CorrectKey", "TrialTime", "TrialNum", "ReinTrialNum", "SampleFR", 
            "SampleTrialType", "CorrectComparisonGroup", "FoilGroup", "Date","ComparisonTrialTime"] # Column headers
        ...
        if operant_box_version:
            if self.exp_phase_num == 0:
                self.stimuli_folder_path = "/home/blaisdelllab/Desktop/Experiments/P035/P035f_Stimuli_familiarized_comparisons" #grabs the stimuli
                self.stimuli_files_list = listdir(self.stimuli_folder_path) #creates a dictionary of the stimuli
            elif self.exp_phase_num == 1:
                self.stimuli_folder_path = "/home/blaisdelllab/Desktop/Experiments/P035/P035f_Stimuli" #grabs the stimuli
                self.stimuli_files_list = listdir(self.stimuli_folder_path) #creates a dictionary of the stimuli
        else: 
            if self.exp_phase_num == 0:
                self.stimuli_folder_path = "/Users/kayleyozimac/Desktop/P035f/P035f_Stimuli_familiarized_comparisons" #grabs the stimuli
                self.stimuli_files_list = listdir(self.stimuli_folder_path) #creates a dictionary of the stimuli
            elif self.exp_phase_num == 1:
                self.stimuli_folder_path = "/Users/kayleyozimac/Desktop/P035f/P035f_Stimuli" #grabs the stimuli
                self.stimuli_files_list = listdir(self.stimuli_folder_path) #creates a dictionary of the stimuli
        ...
        if self.exp_phase_num == 0:
            ...
            # ---- Subphase 1 (start key) using BLOCKS + perfect 3L/3R balancing ----
            comparison_list = [s for s in self.stimuli_files_list if s.startswith("C")]
            comparison_list.sort()
            
            # Track how many left/right uses each stimulus has gotten
            side_counts = {stim: {"left": 0, "right": 0} for stim in comparison_list}
            
            blocks = int(self.max_trials / len(comparison_list))  # 6 blocks of 10
            
            self.stimuli_assignment_dict = {}
            trial_counter = 1
            
            for b in range(blocks):
                block = comparison_list[:]  
                shuffle(block)
            
                for stim in block:
                    left_used  = side_counts[stim]["left"]
                    right_used = side_counts[stim]["right"]
            
                    remaining_left  = 3 - left_used
                    remaining_right = 3 - right_used
            
                    # If both sides still open → weighted random choice
                    ...
                    side_counts[stim][side] += 1
            
                    self.stimuli_assignment_dict[trial_counter] = {
                        "stimuli": stim,
                        "start_key_location": side
                    }
            
                    trial_counter += 1
        
            # ---- Subphase 2 (choice task) plan: smaller vs larger circles; limit ≤ 3 same-side streak ----
            self.choice_stimuli_assignment_dict = {}
            smaller_circle = "correct_choice"
            larger_circle  = "incorrect_choice"
        
            last_pos = None
            streak   = 0
            for tnum, start_key_trial in self.stimuli_assignment_dict.items():
                start_loc = start_key_trial["start_key_location"]  # "left" or "right"
            
                # 50% of the time: comparison matches start-key side
                if random() < 0.5:
                    pos = start_loc
                else:
                    pos = "left" if start_loc == "right" else "right"
            
                # Build dictionary entry
                self.choice_stimuli_assignment_dict[tnum] = {
                    "smaller_circle":  smaller_circle,
                    "larger_circle":   larger_circle,
                    "left_stimulus":   smaller_circle if pos == "left"  else larger_circle,
                    "right_stimulus":  larger_circle  if pos == "left"  else smaller_circle,
                }
            for t in range(1, self.max_trials + 1):
                sk = self.stimuli_assignment_dict.get(t, {})
                ct = self.choice_stimuli_assignment_dict.get(t, {})
                
                stim = sk.get("stimuli", "NA")
                start_loc = sk.get("start_key_location", "NA")
                left = ct.get("left_stimulus", "NA")
                right = ct.get("right_stimulus", "NA")
                
                # Determine which is smaller/larger
                if left == "correct_choice":
                    smaller = "left"
                    larger = "right"
                else:
                    smaller = "right"
                ...
                print(f"Trial {t:02d} | Stim={stim:<10} | StartKey={start_loc:<5} "
                      f"| Smaller={smaller:<5} | Larger={larger:<5}")

                    
        elif self.exp_phase_num == 1: # If P035f experimental trials
...
###### peck counter (sample)
    ...
    def comparison_phase(self):
        ...
        # Load the selected image
            ...
            self.mastercanvas.tag_bind("inactive_sample_key_press",
        ...
        elif self.exp_phase_num == 0:  # Familiarization: start key (FR2) at left or right, then choice task
                trial_info_dict = self.stimuli_assignment_dict[self.current_trial_counter]
                stimulus_file = trial_info_dict['stimuli']
                start_key_location = trial_info_dict['start_key_location']  # "left" or "right"
                self.comparison_start_time = datetime.now()
            
                # Load the image for the comparison (start key)
                selected_image_path = os_path.join(self.stimuli_folder_path, stimulus_file)
                selected_image = Image.open(selected_image_path)
                self.image = ImageTk.PhotoImage(selected_image)
            
                # Coordinates match your comparison locations (same as choice task area)
                if start_key_location == "left":
                    x, y = 160, 550
                else:
                    x, y = 864, 550
            
                # Backdrop + peck logging
                ...
                # Draw black bezel under the key
                oval_radius = 85
                self.mastercanvas.create_oval(
                    x - oval_radius, y - oval_radius, x + oval_radius, y + oval_radius,
                    fill="black", tag="button"
                )
            
                # Show the start-key image at the side location
                self.mastercanvas.create_image(x, y, image=self.image, anchor="center", tag="button")
            
                # FR2: two pecks → move to size-choice subphase
                self.mastercanvas.tag_bind("button", "<Button-1>", self.comp_key_press)
            
                # Keep a handy attribute for logging/consistency if you want it later
                self.comparison_location = start_key_location
    ...
    def choice_task(self):
        if self.exp_phase_num == 0:
            ...
            smaller_circle = choice_trial_info_dict.get("smaller_circle")
            larger_circle = choice_trial_info_dict.get("larger_circle")
            ...
            if not all([smaller_circle, larger_circle, left_stimulus, right_stimulus]):
            ...
            left_x, left_y   = 300, 550
            right_x, right_y = 724, 550
            
            # Define the size of the circles based on their labels
            if left_stimulus == smaller_circle:
      # Draw a larger black circle behind the smaller white circle
              self.mastercanvas.create_oval(left_x - 85, left_y - 85,  # Larger black circle
                                    left_x + 85, left_y + 85,
                                    fill="black", 
                                    tag="left_button_bg")
              self.mastercanvas.create_oval(left_x - 50, left_y - 50,  # Smaller white circle on top
                                    left_x + 50, left_y + 50,
                                    fill="white", 
                                    tag="left_button")
      
              self.mastercanvas.create_oval(right_x - 85, right_y - 85,  # Right side larger white circle
                                    right_x + 85, right_y + 85,
                                    fill="white", 
                                    tag="right_button")
            else:
                self.mastercanvas.create_oval(left_x - 85, left_y - 85,  # Left side larger white circle
                                    left_x + 85, left_y + 85,
                                    fill="white", 
                                    tag="left_button")
      
      # Draw a larger black circle behind the smaller white circle on the right side
                self.mastercanvas.create_oval(right_x - 85, right_y - 85,  # Larger black circle
                                    right_x + 85, right_y + 85,
                                    fill="black", 
                                    tag="right_button_bg")
                self.mastercanvas.create_oval(right_x - 50, right_y - 50,  # Smaller white circle on top
                                    right_x + 50, right_y + 50,
                                    fill="white", 
                                    tag="right_button")
...
###### peck counter (comparison)
    ...
    def comp_key_press(self, event):
        ...
        if self.comparison_key_presses >= self.comparison_FR:
            # 1-second delay before choice task appears
            self.clear_canvas()
...
#######
   ...
   #     else: 
    ...
    # Phase 0 (familiarization and choice task)
        if self.exp_phase_num == 0:
            # Get current trial's choice task info
        ...
        # Assign LComp and RComp directly as "smaller_circle" or "larger_circle"
            LComp = "smaller_circle" if left_stimulus == "correct_choice" else "larger_circle"
            RComp = "smaller_circle" if right_stimulus == "correct_choice" else "larger_circle"
    
            # Define CorrectKey (CompStimulus, I know it's weird) as "smaller_circle" for all trials in Phase 0
            stimulus_file = trial_info_dict.get('stimuli')
            CompStimulus = "smaller_circle"

        # Phase 1 logic (sample, comparison, and foil)
        if self.exp_phase_num == 1:
            trial_info_dict = self.stimuli_assignment_dict.get(self.current_trial_counter, {})
        
            # Both are the same in Phase 1
            stimulus_file = trial_info_dict.get("comparison", "NA")
            CompStimulus = trial_info_dict.get("comparison", "NA") 
        ...
        header_list = [
            ...
            "TrialSubStage", "SampleStimulus", "CompStimulus", "LComp", "RComp", 
            "CorrectKey", "TrialTime", "TrialNum", "ReinTrialNum", "SampleFR", 
            "SampleTrialType", "CorrectComparisonGroup", "FoilGroup", "Date",
            "ComparisonTrialTime"] # Column headers
    ...
    def write_comp_data(self, SessionEnded):
        ...
        if self.record_data : # If experimenter has choosen to automatically record data in seperate sheet:
            myFile_loc = f"{self.data_folder_directory}/{self.subject_ID}/{self.subject_ID}_{self.start_time.strftime('%Y-%m-%d_%H.%M.%S')}_P035f_data-Phase{self.exp_phase_num}.csv" # location of written .csv
