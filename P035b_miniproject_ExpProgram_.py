...
(VR5 with bounds from 6 to 10, but this was built to be maniputable),two comparisons 
...
class ExperimenterControlPanel(object):
    ...
    def __init__(self):
        ...
        if operant_box_version:
            ...
            self.data_folder = "P035b_data" # The folder within Documents where subject data is kept
...
class MainScreen(object):
    ...
    def __init__(self, subject_ID, record_data, data_folder_directory,
        ...
        self.phase_start_time = None #For comparison phase timer 
        ...
        self.hopper_duration = 6000 # Duration of food access (ms)
        # Set up FR (2) 
        ...
        # Declare variables that will be added to data sheet
        self.current_trial_counter = 0 # counter for current trial in session
        
        self.click_counter = 0  # Initialize click count, this will hopefully be called once
        # I figure it out lol

        # These are additional "under the hood" variables that need to be declared
        # Do if else statement for max trial number
        if self.exp_phase_num == 0:
            self.max_trials = 60 # Max number of trials within a session
        ...
        if self.exp_phase_num == 0: # For familiarization phase
            ...
            # ^ counts how many times each stimulus was presented on left/right so we can keep track
            ...
            # {
            #   1 : {"stimuli": "C8.bmp", "location": "right"},
            #   2 : {"stimuli": "C1.bmp", "location": "left"},
            #   .....
            # }
            
            self.stimuli_assignment_dict = {} # This dictionary will contain all stimulus assignments
            trial_counter = 1
            location_list = ["left", "right"] * int(len(self.stimuli_files_list) / 2) # Should be 5
            
            # Next, build a while loop to counter balance left and right stimulus assignments
            while trial_counter <= self.max_trials:
                # Shake our bag of 10 marbles
                    shuffle(location_list)
                    shuffle(self.stimuli_files_list)
                
                    for s in self.stimuli_files_list:
                        index_of_current_stimulus = self.stimuli_files_list.index(s)
                        self.stimuli_assignment_dict[trial_counter] = {"stimuli" : s,
                                                                   "location": location_list[index_of_current_stimulus]}
            ...
            self.stimuli_assignment_dict = {} # This dictionary will contain all stimulus assignments
            counter = 0
            trial_num_list = list(range(1, self.max_trials + 1)) # Plus one because stopping point!
            shuffle(trial_num_list) # Shuffle numbers!

            # Next, build a while loop to build all stimuli assignments
            while counter < self.max_trials: # This assumes that the max trials is equally divisible by the number of samples
                for samp in self.stimuli_files_list:
                    if samp[0] == "S":
                        sample = samp
                        ...
                        self.stimuli_assignment_dict[trial_num_list[counter]] = trial_dict
...
###### peck counter (sample)
    ...
    # Check if the trial number has changed
        if self.current_trial_counter != self.last_written_trial_num:
            self.comparison_start_time = datetime.now()  # Restart the timer if the trial number has changed
        ...
        if self.exp_phase_num == 1: # For familiarization phase
...
#######
  ...
  #          round((time() - self.trial_start  #comparison trial time
        ])
        header_list = [
            "SessionTime", "ExpPhase", "Subject", "Xcord", "Ycord", "Event",
            "TrialSubStage", "SampleStimulus", "LComp", "RComp", "CorrectKey",
