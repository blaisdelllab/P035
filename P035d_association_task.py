...
resulted in the beginning of Subphase 2.
    ...
    Subphase 2 (comparison). A paired comparison to the distractor sample is presented
below the sample. Two (?) pecks at the stimulus resulted in the presentation of:
...
class MainScreen(object):
    ...
    def __init__(self, subject_ID, record_data, data_folder_directory,
        ...
        if operant_box_version: 
            ...
            # Force the window onto the second screen (e.g., touchscreen to the right of primary)
            self.root.geometry("1024x768+1024+0")  # Adjust +X+Y as needed for your setup
        ...
        header_list = [
            ...
            "ComparisonFamiliarity", "Date", "ComparisonTrialTime"]
        ...
        if self.exp_phase_num in [0, 1]:  # Phase 0 and Phase 1 use the same stimuli
            ...
            if operant_box_version:
                self.distractor_stimuli_path = os_path.join(base_path, "P035d/P035d_distractor_samples")
            else:
  ...
  #              print(self.choice_stimuli_assignment_dict)
            ...
            assert len(comparison_list) == 10 and len(distractor_list) == 10, "Phase 1 must have 10 comparisons + 10 distractors"

            # 70 training trials (as in Phase 1)
            training_trials = []
            for stim, distractor in zip(comparison_list, distractor_list):
                for _ in range(7):
                    ...
                    training_trials.append({
                        ...
                        "distractor_sample": distractor,
                        "sample_FR": sample_FR,
                        "comparison_location": side
                    })

            # 30 test trials: 3 per familiarized comparison
            test_trials = []
            for stim, distractor in zip(comparison_list, distractor_list):
                for _ in range(3):
                    # Select a different familiarized comparison as distractor
                    foil = choice([s for s in comparison_list if s != stim])
                    side = choice(["left", "right"])
                    sample_FR = choice(range(3, 9))
                    test_trials.append({
                        "trial_type": "test",
            ...
            full_trial_list = training_trials + test_trials
            shuffle(full_trial_list)
            self.stimuli_assignment_dict = {}
            for i, trial in enumerate(full_trial_list, 1):
                self.stimuli_assignment_dict[i] = trial
            ...
            for t, v in self.stimuli_assignment_dict.items():
                if v["trial_type"] == "training":
                    ...
                    print(f"Trial {t} [TEST]: Distractor = {v['distractor_sample']}, Comp = {v['paired_comparison']}, Foil = {v['familiar_distractor']}, Side = {v['comparison_location']}")
                        ...
                        # Build choice task mapping for Phase 1
            self.choice_stimuli_assignment_dict = {}
    ...
    def comparison_phase(self):
        ...
        if self.exp_phase_num == 1:
            ...
            if trial_type == "test":
                ...
                self.mastercanvas.tag_bind("comparison_button", "<Button-1>", self.correct_choice)
                ...
                self.mastercanvas.tag_bind("foil_button", "<Button-1>", self.incorrect_choice)
            else:
...
###### peck counter (comparison)
    ...
    def comp_key_press(self, event):
        ...
        if self.comparison_key_presses >= self.comparison_FR:
            self.clear_canvas()  # Clear the screen during delay
            if self.exp_phase_num == 0 or 1:
                self.root.after(1000, self.choice_task)  # Delay before Phase 1 choice task
                if not operant_box_version or self.subject_ID == "TEST":
                    self.mastercanvas.create_text(512, 384,
                                                  fill="white",
                ...
                self.root.after(1000, self.choice_task)  # Delay before Phase 2 MTS task
        else:
            self.comparison_phase()
...
#######
   ...
   #     else: 
    ...
    # Phase 0 (familiarization and choice task)
        ...
        if self.trial_stage == 1 and self.comparison_start_time is not None:
            comparison_trial_time = (datetime.now() - self.comparison_start_time).total_seconds()
        else:
            comparison_trial_time = "NA"
