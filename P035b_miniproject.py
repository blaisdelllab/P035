...
#######
   ...
   #     else: 
    ...
    def write_data(self, event, outcome):
        ...
        # Ignore if current trial counter is 0
        if self.current_trial_counter == 0:
            return

        # Retrieve trial information directly from the stimuli assignment dictionary
        if self.current_trial_counter in self.stimuli_assignment_dict:
            trial_info_dict = self.stimuli_assignment_dict[self.current_trial_counter]
        else:
            trial_info_dict = self.stimuli_assignment_dict[self.current_trial_counter - 1]
